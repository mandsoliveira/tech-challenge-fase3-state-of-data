...
Especificação completa em `.kiro/specs/state-of-data-fase3/`.
...
# 6. Material executivo (usa python-pptx, disponível no venv principal)
...
| Diagrama de arquitetura | `arquitetura/arquitetura_sod_fase3.drawio` + PNG e PDF |
...
| Evidência de execução | `output/evidencias/execucao_glue.json` e `consultas_athena.json` |
...
S3 Gold    ---- Parquet, 7 tabelas agregadas, uma por pergunta de negócio
     v
Glue Data Catalog --> Athena (SQL) --> gráficos --> material executivo
```
...
│       └── silver_para_gold.py   # as 7 agregações
